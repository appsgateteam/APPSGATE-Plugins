...
class DocumentTestCase(common.TransactionCase):
    ...
    def test_security_without_groups(self):
        ...
        for user_id in data.user_ids:
            document_id = self.env["ag_quality_docs.document"].create({
            ...
            for stage_id in data.stage_ids:
                ...
                if user_id.has_group("ag_quality_docs.group_ag_quality_docs_manager"):
                    has_read_in_stage = True
                    has_write_in_stage = stage_id.has_write_access_for_managers
                elif user_id.has_group("ag_quality_docs.group_ag_quality_docs_author"):
                ...
                elif user_id.has_group("ag_quality_docs.group_ag_quality_docs_user"):
    ...
    def test_security_with_groups(self):
        ...
        for user_id in data.user_ids:
            ...
            for stage_id in data.stage_ids:
                ...
                group = self.env["ag_security.access_groups"].create({
                    "name": "WRONG READ",
                    "perm_read": True,
                })
    ...
    def test_workflow_security(self):
        ...
        for user_id in data.user_ids:
            ...
            # Test wrong group
            group = self.env["ag_quality_docs.groups"].create({
    ...
    class TestData():
        def __init__(self, env):
            self.manager = env.ref("ag_quality_docs.manager_demo")
            self.author = env.ref("ag_quality_docs.author_demo")
            self.user = env.ref("base.user_demo")
            
            self.user_ids = self.manager | self.author | self.user
            
            self.stage_ids = env["ag_quality_docs.stage"].search([("sequence", ">", 1000)])

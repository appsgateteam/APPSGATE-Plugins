...
class ActionTestCase(DocumentsBaseCase):
    ...
    def setUp(self):
        ...
        self.action = self.env['ag_dms_actions.action'].sudo()
        ...
        self.test_category = self.env['ag_dms.category'].create({
            'name': "Test Category",
        })
        self.test_tag = self.env['ag_dms.tag'].create({
            'name': "Test Tag",
        })
    ...
    def test_file_action(self):
        ...
        test_action.trigger_actions(self.browse_ref("ag_dms.file_01_demo").ids)
        self.assertTrue(self.env['res.partner'].sudo().search_count([]) > counter)
        
    @setup_data_function(setup_func='_setup_test_data')
    def test_server_action(self):
        action = self.browse_ref("ag_dms.action_dms_attachment_migrate")
        test_action = self.action.create({

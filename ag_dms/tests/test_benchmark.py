...
from odoo.addons.ag_utils.tests.common import track_function
...
class BenchmarkTestCase(common.SavepointCase): 
    ...
    def _clean_existing_records(cls):
        cls.env['ag_dms.category'].search([]).unlink()
        cls.env['ag_dms.directory'].search([]).unlink()
        cls.env['ag_dms.storage'].search([]).unlink()
        cls.env['ag_dms.tag'].search([]).unlink()   
    ...
    def _setup_benchmark_data(cls):
        cls._load('ag_dms', 'tests', 'data', 'ag_dms.category.csv')
        cls._load('ag_dms', 'tests', 'data', 'ag_dms.storage.csv')
        cls._load('ag_dms', 'tests', 'data', 'ag_dms.tag.csv')
        cls._load('ag_dms', 'tests', 'data', 'ag_dms.directory.csv')
        cls._load('ag_dms', 'tests', 'data', 'ag_dms.file.csv')
    ...
    def test_file_search_benchmark(self):
        ...
        model =  self.env['ag_dms.file'].with_context(bin_size=True)
    ...
    def test_directory_search_benchmark(self):
        demo_uid = self.browse_ref("base.user_demo").id
        admin_uid = self.browse_ref("base.user_admin").id
        model =  self.env['ag_dms.directory'].with_context(bin_size=True)
        args = [[[[]], {'limit': 1}], [[[]], {'limit': 80}], [[[]], {'limit': 500}], [[[]]]]
    ...
    def test_file_search_read_profile_admin(self):
        ...
        def profile_function(model):
            model.search_read([])
        admin_uid = self.browse_ref("base.user_admin").id
        model =  self.env['ag_dms.file'].sudo(admin_uid)

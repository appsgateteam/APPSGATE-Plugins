...
class Document(models.Model):
    ...
    _name = "ag_quality_docs.document"
    _description = "QMS Document"
    _inherit = ["mail.thread", "mail.activity.mixin", "ag_security.mixins.access_groups"]
    ...
    stage_id = fields.Many2one(
        "ag_quality_docs.stage",
        string="Stage",
        ondelete="RESTRICT",
        index=True,
        readonly=True,
        default=lambda r: r.env["ag_quality_docs.stage"].get_first_stage(),
        track_visibility="onchange"
    )
    ...
    read_ids = fields.One2many(
        "ag_quality_docs.read",
        "document_id",
        string="Already read by",
        groups="ag_quality_docs.group_ag_quality_docs_manager"
    )
    ...
    def _compute_has_right_for_prev_stage(self):
        for record in self:
            ...
            if stage_new:
                ...
                enable_workflow_prev_stage = get_param("ag_quality_docs.enable_workflow_prev_stage")
                has_group = self.env.user.has_group("ag_quality_docs.group_ag_quality_docs_manager")
    ...
    def _compute_has_right_for_next_stage(self):
        for record in self:
            ...
            if stage_new:
                if self.env.user.has_group("ag_quality_docs.group_ag_quality_docs_manager"):
                    has_right = True
    ...
    def toggle_read(self):
        if not self.is_read:
            self.env["ag_quality_docs.read"].create({
                "document_id": self.id,
                "user_id": self.env.user.id    
            })
        else:
            self.env["ag_quality_docs.read"].search([
                "&",
                ("document_id", "=", self.id),
                ("user_id", "=", self.env.user.id)
            ]).unlink()
    ...
    def _search_is_read(self, operator, value):
        if operator == "=":
            records = self.env["ag_quality_docs.read"].sudo().search([("user_id", "=", self.env.user.id)]).mapped("document_id.id")
    ...
    def action_create_document_from_document(self):
        ...
        return {
            ...
            "view_mode": "form",
            "res_model": "ag_quality_docs.document",
            "target": "current",
            "context": context
        }
    ...
    def _apply_ir_rules(self, query, mode='read'):
        user = self.env.user
        if user.has_group("ag_quality_docs.group_ag_quality_docs_manager"):
            pass
        elif user.has_group("ag_quality_docs.group_ag_quality_docs_author"):
            if mode == "read":
                Stage = self.env["ag_quality_docs.stage"]
                stages = Stage.search([
                    "|",
                ...
                if stages:
                    ...
                    clause = '"ag_quality_docs_document".stage_id IN {}'.format(stage_ids)
                else:
                    clause = '1 != 1'
            
                query.where_clause += [clause]
        elif user.has_group("ag_quality_docs.group_ag_quality_docs_user"):
            if mode == "read":
                Stage = self.env["ag_quality_docs.stage"]
    ...
    def check_access_rule(self, operation):
        ...
        if operation == "write":
            if user.id == 1:
                pass
            elif user.has_group("ag_quality_docs.group_ag_quality_docs_manager"):

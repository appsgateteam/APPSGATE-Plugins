...
from odoo.addons.ag_dms_field.fields import binary

_logger = logging.getLogger(__name__)

class Document(models.Model):
    
    _inherit = "ag_quality_docs.document"
    ...
    def _get_file_directory(self):
        try:
            ...
            file_directory = get_param("ag_quality_docs_dms.file_directory")
        except:
            try:
    ...
    def _get_viewer_file_directory(self):
        try:
            get_param = self.env["ir.config_parameter"].sudo().get_param
            viewer_file_directory = get_param("ag_quality_docs_dms.viewer_file_directory")
        except:
            try:
                get_param = self["ir.config_parameter"].sudo().get_param
                viewer_file_directory = get_param("ag_quality_docs_dms.viewer_file_directory")
    ...
    def _compute_user_can_only_see_viewer_file(self):
        normal_user = not self.env.user.has_group("ag_quality_docs.group_ag_quality_docs_author")
        get_param = self.env["ir.config_parameter"].sudo().get_param

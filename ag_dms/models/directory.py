...
class Directory(models.Model):
    ...
    _name = 'ag_dms.directory'
    _description = "Directory"
    
    _inherit = [
        'ag_utils.mixins.hierarchy',
    ...
    tags = fields.Many2many(
        ...
        relation='ag_dms_directory_tag_rel', 
        domain="""[
            '|', ['category', '=', False],
    ...
    user_stars = fields.Many2many(
        ...
        relation='ag_dms_directory_star_rel',
        column1='did',
        column2='uid',
        string='Stars')
    ...
    def search_panel_select_multi_range(self, field_name, **kwargs):
        ...
        if field_name == 'tags':
            sql_query = '''
                ...
                LEFT JOIN ag_dms_directory_tag_rel r ON t.id = r.tid 
    ...
    def _compute_size(self):
        sudo_model = self.env['ag_dms.file'].sudo()
        for record in self:
    ...
    def _inverse_starred(self):
        starred_records = self.env['ag_dms.directory'].sudo()
        not_starred_records = self.env['ag_dms.directory'].sudo()
        for record in self:
    ...
    def copy(self, default=None):
        ...
        if 'root_storage' in default:
            storage = self.env['ag_dms.storage'].browse(default['root_storage'])
    ...
    def unlink(self):
        if self and self.check_access('unlink', raise_exception=True):
            ...
            if self.env['ag_dms.file'].sudo().search(domain):
                raise AccessError(_("A file is locked, the folder cannot be deleted.")) 
            self.env['ag_dms.file'].sudo().search([('directory', 'child_of', self.ids)]).unlink()

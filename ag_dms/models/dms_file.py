...
class File(models.Model):
    ...
    def forbidden_extensions(self):
        params = self.env['ir.config_parameter'].sudo()
        forbidden_extensions = params.get_param('ag_dms.forbidden_extensions', default="")
    ...
    def copy(self, default=None):
        ...
        if 'directory' in default:
            directory = self.env['ag_dms.directory'].sudo().browse(default['directory'])
    ...
    def _create_reference(self, settings, path, filename, content):
        ...
        if settings.read(['save_type']) and settings.save_type == 'database':
            return self.env['ag_dms.data_database'].sudo().create({'data': content})
        return None

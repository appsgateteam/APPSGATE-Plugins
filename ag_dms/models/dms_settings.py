...
class Settings(models.Model):
    ...
    _name = 'ag_dms.settings'
    _description = "ag Documents Settings"
    _inherit = 'ag_dms.model'
    ...
    def _search_top_directories(self, operator, operand):
        directories = self.env['ag_dms.directory'].search([('name', operator, operand)])
    ...
    def _search_top_files(self, operator, operand):
        files = self.env['ag_dms.file'].search([('name', operator, operand)])
    ...
    def _compute_top_directories(self):
        for record in self: 
            access_ids = self.env['ag_dms.directory']._get_complete_access_ids("read")

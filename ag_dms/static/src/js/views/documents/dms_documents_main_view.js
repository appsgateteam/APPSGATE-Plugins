...
odoo.define('ag_dms_views.DocumentsMainView', function(require) {
"use strict";
...
var utils = require('ag_web_utils.common');
var mimetype = require('ag_web_utils.mimetype');

var Widget = require('web.Widget');
var Dialog = require('web.Dialog');

var DocumentTreeView = require('ag_dms_views.DocumentTreeView');
var PreviewHelper = require('ag_dms_preview_file.PreviewHelper');
...
var DocumentTreeMainView = DocumentTreeView.extend({
	cssLibs: [
        '/ag_web_utils/static/lib/jquery-splitter/css/jquery.splitter.css',
    ],
    jsLibs: [
        '/ag_web_utils/static/lib/jquery-splitter/js/jquery.splitter.js',
    ],
    custom_events: _.extend({}, DocumentTreeView.prototype.custom_events, {
    	tree_ready: '_treeReady',
    	tree_changed: '_treeChanged',
    }),
	template: 'ag_dms.DocumentTreeView',
    ...
    _update_cp: function() {
    	var self = this;
    	if (!this.$buttons) {
            this.$buttons = $(QWeb.render('ag_dms.DocumentTreeViewButtons', {
                widget: this,
            }));
    	...
    	if (!this.$pager) {
            this.$pager = $(QWeb.render('ag_dms.DocumentTreeViewActions', {
                widget: this,
            }));
    	...
    	if (!this.$switch_buttons) {
            this.$switch_buttons = $(QWeb.render('ag_dms.DocumentTreeViewOptions', {
                widget: this,
            }));
        }
    	if (!this.$searchview) {
            this.$searchview = $(QWeb.render('ag_dms.DocumentTreeViewSearch', {
                widget: this,
            }));
    ...
    _preview_node: function(node) {
    	var self = this;
    	if(node.data && node.data.odoo_model === "ag_dms.file") {
    ...
    _create_selected_node: function() {
    	var node = this.controller.getSelectedItem();
    	if(node) {
    		var parent = node.data.odoo_model === "ag_dms.file" ? this.controller.getParentNode(node) : node;
    ...
    _show_help: function() {
    	...
    	var dialog = new Dialog(this, {
            size: 'medium',
            buttons: buttons,
            $content: $(QWeb.render('ag_dms.DocumentHelpDialogContent')),
            title: _t("Help"),
        });
    	dialog.open();
    },
...
core.action_registry.add('ag_dms_views.documents', DocumentTreeMainView);

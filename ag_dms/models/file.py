...
from odoo.addons.ag_utils.tools import file
from odoo.addons.ag_security.tools.security import NoSecurityUid
...
class File(models.Model):
    ...
    _inherit = [
        'ag_security.mixins.access_rights',
        'ag_security.mixins.locking',
        'ag_dms.mixins.thumbnail',
    ]
    ...
    def _get_binary_max_size(self):
        ...
        return int(get_param('ag_web_utils.binary_max_size', default=25))
     
    @api.model
    def _get_forbidden_extensions(self):
        get_param = self.env['ir.config_parameter'].sudo().get_param
        extensions = get_param('ag_dms.forbidden_extensions', default="")
    ...
    def search_panel_select_range(self, field_name, **kwargs):
        ...
        if directory_id and field_name == 'directory':
            ...
            values = self.env['ag_dms.directory'].search_read(
    ...
    def search_panel_select_multi_range(self, field_name, **kwargs):
        ...
        if field_name == 'tags':
            sql_query = '''
                ...
                FROM ag_dms_tag t
                JOIN ag_dms_category c ON t.category = c.id
                LEFT JOIN ag_dms_file_tag_rel r ON t.id = r.tid 
    ...
    def _compute_migration(self):
        storage_model = self.env['ag_dms.storage']
    ...
    def _get_directories_from_database(self, file_ids):
        if not file_ids:
            return self.env['ag_dms.directory']
        sql_query = '''
            SELECT directory 
            FROM ag_dms_file
        ...
        return self.env['ag_dms.directory'].browse(result)
    ...
    def _read_group_process_groupby(self, gb, query):
        ...
        directories = self.env['ag_dms.directory'].with_context(prefetch_fields=False).search([])
        if directories:
    ...
    def check_directory_access(self, operation, vals={}, raise_exception=False):
        ...
        if 'directory' in vals and vals['directory']:
            records = self.env['ag_dms.directory'].browse(vals['directory'])
        else:

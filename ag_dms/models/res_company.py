...
class ResCompany(models.Model):
    ...
    def action_open_documents_onboarding_storage(self):
        return self.env.ref('ag_dms.action_dms_storage_new').read()[0]
    
    @api.model
    def action_open_documents_onboarding_directory(self):
        storage = self.env['ag_dms.storage'].search([], order="create_date desc", limit=1)
        action = self.env.ref('ag_dms.action_dms_directory_new').read()[0]
    ...
    def action_open_documents_onboarding_file(self):
        directory = self.env['ag_dms.directory'].search([], order="create_date desc", limit=1)
        action = self.env.ref('ag_dms.action_dms_file_new').read()[0]
        action['context'] = {**self.env.context, **{

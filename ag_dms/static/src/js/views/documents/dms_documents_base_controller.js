...
odoo.define('ag_dms_views.DocumentsController', function(require) {
"use strict";
    ...
    refresh: function(message) {
    	...
    	if(message && message.ids && message.model && !message.create) {
    		var nodes = _.each(_.keys(_.chain(message.ids).map(function(id) {
    			...
    			if(message.model === 'ag_dms.directory') {
    		...
    		if(id && message.model) {
    			var field = message.model === 'ag_dms.directory' ? 'parent_directory' : 'directory';
    			this._rpc({
    				 fields: [field],
    				 domain: [['id', '=', id]],
    				 model: message.model,

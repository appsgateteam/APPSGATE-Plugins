...
odoo.define('ag_web_share.dialog', function(require) {
"use strict";
...
var ShareDialog = Dialog.extend({
	init: function (parent, options) {
		...
		options = _.defaults(options || {}, {
            ...
            size: 'smale',
            dialogClass: 'ag_share_dialog_body',
            $content: $(QWeb.render('ag_web_share.ShareDialog', {
            	widget: this,
	...
	willStart: function () {
        ...
        return this._super.apply(this, arguments).then(function () {
        	self.$modal.find('.modal-dialog').addClass("ag_share_dialog");
        	self.$modal.find('.modal-title').prepend($('<i class="fa fa-share-alt"/>'));
        	self.$content.find('.ag_share_buttons_whatsapp').attr("href",
            		'whatsapp://send?text=' + encodeURIComponent(self.url));
            self.$content.find('.ag_share_buttons_email').attr("href",
            		'mailto:?subject=' + _t("Share") + '&body=' + encodeURIComponent(self.url));
        	self.$content.find('.ag_share_buttons_chat').click(function(e) {
        	    e.preventDefault();
        	    e.stopPropagation();
        	    self.newMessage(self.url);
            });
        	self.$content.find('.ag_share_buttons_copy').click(function(e) {
        	    e.preventDefault();
        	    e.stopPropagation();
            	self.copyClipboard(self.url);
            });
            self.$content.find('.ag_share_chat_send').click(function(e) {
        	    e.preventDefault();
        	    e.stopPropagation();
        	    self.newMessage(self.url);
            });
    ...
    updateUser: function() {
    	...
    	self._rpc({
        	...
        	self.$content.find('#user').append($(QWeb.render('ag_web_share.ShareUsers', {
            	widget: self,
            	users: users,
            })));
        	self.$content.find('.ag_share_chat_user').click(function(e) {
    ...
    updateChannels: function() {
        ...
        chat_manager.get_channels_preview(channels).then(function(channels) {
        	self.$content.find('#channel').append( $(QWeb.render('ag_web_share.ShareChannels', {
            	widget: self,
            	channels: channels,
            })));
        	self.$content.find('.ag_share_chat_channel').click(function(e) {
        ...
        chat_manager.get_channels_preview(chats).then(function(channels) {
        	self.$content.find('#chat').append( $(QWeb.render('ag_web_share.ShareChatItems', {
            	widget: self,
            	channels: channels,
            })));
        	self.$content.find('.ag_share_chat_chat').click(function(e) {
        		var channel_id = $(e.currentTarget).data('channel_id');
        		self.postMessage(self.formatUrl(self.url), channel_id);
        	});
    	});

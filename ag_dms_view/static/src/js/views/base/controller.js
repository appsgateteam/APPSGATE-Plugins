...
odoo.define('ag_dms_view.DocumentsController', function(require) {
"use strict";
...
var files = require('ag_web_utils.files');
var async = require('ag_web_utils.async');
var mimetype = require('ag_web_utils.mimetype');

var Widget = require('web.Widget');

var FileUpload = require('ag_dms_mixins.FileUpload');
var DocumentDropFileDialog = require('ag_dms_dialogs.DocumentDropFileDialog');
var DocumentDropFilesDialog = require('ag_dms_dialogs.DocumentDropFilesDialog');
var DocumentFileInfoDialog = require('ag_dms_dialogs.DocumentFileInfoDialog');
var DocumentDirectoryInfoDialog = require('ag_dms_dialogs.DocumentDirectoryInfoDialog');
...
var DocumentsController = Widget.extend(FileUpload, {
    ...
    refresh: function(message) {
    	...
    	if(message && message.ids && message.model && !message.create) {
    		...
    		if(id && message.model) {
    			var field = message.model === 'ag_dms.directory' ?
    ...
    getSelectedDirectory: function() {
    	...
    	var directories = _.filter(selected, function(node) {
    		return node.data.odoo_model === "ag_dms.directory";
    	});
    ...
    _checkCallback: function (operation, node, parent, position, more) {
    	if(operation === "copy_node" || operation === "move_node") {
    		...
    		if(parent.data && parent.data.odoo_model === "ag_dms.settings") {
    			return false;
            }
    		// prevent moving a child to a file
    		if(parent.data && parent.data.odoo_model === "ag_dms.file") {
    			return false;
            }
    	}
    ...
    _checkSelect: function(node, event) {
    	if(this.params.filesOnly && node.data.odoo_model !== "ag_dms.file") {
    		return false;
    	}
    	return !(node.parent === '#' && node.data.odoo_model === "ag_dms.settings");
    },
...
//    	$("#menuContinenti").prop('disabled', function (_, val) { return ! val; });
    ...
    _openInfo: function(node) {
		if(node.data.odoo_model == "ag_dms.file") {
    ...
    _moveNode: function(ev) {
		...
		} else if(node.data.odoo_model === "ag_dms.directory") {
    ...
    _copyNode: function(ev) {
		...
		if(original.data.odoo_model === "ag_dms.file") {
			vals.directory = parent.data.odoo_id;
		} else if(original.data.odoo_model === "ag_dms.directory") {
			vals.parent_directory = parent.data.odoo_id;
		}
		this._rpc({

...
class OnboardingController(http.Controller):
    ...
    def document_onboarding_directory(self):
        ...
        check = request.env.user.has_group('ag_dms.group_dms_manager')
        if check and not closed:
            return {
                'html': request.env.ref('ag_dms.document_onboarding_directory_panel').render({
    ...
    def document_onboarding_file(self):
        ...
        if check and not closed:
            return {
                'html': request.env.ref('ag_dms.document_onboarding_file_panel').render({
                    'state': company.get_and_update_documents_onboarding_state(),
                    'company': company,
                })
            }

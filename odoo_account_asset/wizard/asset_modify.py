...
class AssetModify(models.TransientModel):
    ...
    def _get_asset_method_time(self):
        if self.env.context.get('active_id'):
            asset = self.env['account.asset.asset.custom'].browse(self.env.context.get('active_id'))
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        if active_model == 'account.asset.asset.custom' and asset_id:
            asset = self.env['account.asset.asset.custom'].browse(asset_id)
    ...
    def default_get(self, fields):
        ...
        if self.env.context.get('active_id'):
            active_asset = self.env['account.asset.asset.custom'].browse(self.env.context.get('active_id'))
    ...
    def modify(self):
        ...
        tracked_fields = self.env['account.asset.asset.custom'].fields_get(['method_number', 'method_period', 'method_end'])

...
class Base(models.AbstractModel):
    ...
    def unlink(self):
        file_ids = set()
        if 'ag_dms.file' in self.env:
            query = 'SELECT id FROM ag_dms_file WHERE reference_model=%s AND reference_id IN %s'
        ...
        if file_ids and 'ag_dms.file' in self.env:
            self.env['ag_dms.file'].sudo().browse(list(file_ids)).unlink()
...
class BaseModelLocking(models.AbstractModel):
    ...
    _name = 'ag_security.locking'
    _description = 'ag Locking Model'
    _inherit = 'ag_utils.model'
    ...
    def lock(self, user=None, operation=None, *largs, **kwargs):
        ...
        for record in self:
            ...
            if lock and lock.operation and lock.operation == operation:
                ...
                lock = self.env['ag_security.lock'].sudo().create({
    ...
    def unlock(self, *largs, **kwargs):
        for record in self:
            locks = self.env['ag_security.lock'].sudo().search(
    ...
    def unlock_operation(self, operation, *largs, **kwargs):
        locks = self.env['ag_security.lock'].sudo().search([('operation', '=', operation)])
        references = [
    ...
    def is_locked(self, *largs, **kwargs):
        self.ensure_one()
        lock = self.env['ag_security.lock'].sudo().search(
